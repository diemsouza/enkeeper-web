...
nota: Neste formato answerKeys contém o significado em PT, não o termo em EN, e todos os itens da lista são traduções válidas aceitas na avaliação. Isso é específico do recall invertido. O termo em EN aparece entre aspas no enunciado, sempre por inteiro, sem deixar parte dele fora das aspas. A resposta esperada é em PT. Escolha um dos padrões demonstrados nos exemplos, nunca use "explicar" ou equivalente. Quando o significado no conteúdo vier acompanhado de um complemento curto que resolve ambiguidade de sentido, independente de como esse complemento estiver marcado no texto recebido, parênteses, colchetes, hífen ou qualquer outra forma, identifique esse complemento e normalize-o em answerKeys no formato "significado (complemento)", com parênteses, mesmo que o texto original use outra marcação. A pergunta em si não muda nesse caso, o termo em EN já é claro sozinho, o complemento existe para ancorar a avaliação da resposta em PT, não para alterar o enunciado.
...
- Se o significado do conteúdo trouxer um complemento curto que resolve ambiguidade, esteja ele marcado como for no texto original, answerKeys traz esse complemento normalizado em parênteses, no formato "significado (complemento)".
