...
> Versão 7 — Abril 2026 (nome definido + modelo trial/pro)
...
Apenas dois planos: **Trial** e **Pro**. Sem tier gratuito permanente.

### Trial — 1 dia, experiência completa de Pro

- 1 dia de uso pleno do produto
- Materiais ilimitados durante o trial (com caps técnicos invisíveis)
- Prática completa: 8-12 mensagens no dia
- Evolução do dia ao final
- Texto, áudio, imagem, PDF — todos liberados
- Após expirar: conta trancada até converter

Filosofia: trial é demonstração honesta. Pessoa vive a experiência completa de Pro por 1 dia, sente o valor, decide se paga. Não é teaser nem versão capada — é o produto inteiro, por tempo limitado.

Por que trial em vez de free permanente:

- Free permanente vira freemium trap: 90% dos usuários ficam felizes com o limite gratuito e nunca pagam, gerando custo recorrente sem receita
- Trial finito identifica quem realmente é cliente. Quem vai pagar, paga rápido. Quem não vai, não paga nunca
- Produto de prática contínua tem ciclo curto de valor — pessoa sente no dia 1 ou não sente nunca
- Custo controlado: trial de 1 dia custa ~R$0,80 fixo por usuário, escala bem mesmo com 10k testando

Trial estendido como ferramenta de campanha:

- Padrão: 1 dia
- Indicação de professor parceiro: 3 dias
- Campanha promocional sazonal: 7 dias
- Beta testers / conhecidos / cortesia permanente: data de expiração distante (estilo 2099-12-31)

Tudo controlado pelo mesmo campo `planExpiresAt`. Sem código novo.

### Pro — R$19,90/mês recorrente

- Materiais ilimitados (com caps técnicos invisíveis: 5 materiais/dia)
- Prática diária com 8-12 mensagens/dia
- Evolução semanal (domingo)
- Texto, áudio, imagem, PDF
- Histórico permanente da prática
- 1 material ativo por vez — manda novo, substitui o anterior automaticamente; antigos ficam acessíveis em `/materiais`

### Pro — R$19,90/30 dias avulso (Pix)

Mesmo preço do recorrente. Sem fricção pra quem quer um mês pontual (estudante de exame, prova específica). Após 30 dias, conta trancada até nova compra ou assinatura.

### Estrutura de dados (MVP)

Mantém o schema simples já em produção:

```
planCode: trial | pro
planStatus: active | canceled | past_due | expired
planExpiresAt: timestamp
```

Mapeamento por cenário:

| Cenário                                        | planCode | planStatus | planExpiresAt        |
| ---------------------------------------------- | -------- | ---------- | -------------------- |
| Usuário novo                                   | trial    | active     | createdAt + 1d       |
| Trial expirou                                  | trial    | expired    | (no passado)         |
| Trial estendido (campanha)                     | trial    | active     | createdAt + 3-7d     |
| Cortesia permanente (founder, beta, parceiros) | trial    | active     | 2099-12-31           |
| Pagou Pro                                      | pro      | active     | paidAt + 30d         |
| Pro cancelou                                   | pro      | canceled   | (data do cancel)     |
| Pro com falha de cobrança                      | pro      | past_due   | (data do vencimento) |

Por que cortesia fica como `trial` e não como `pro`:

- Separa cortesia de receita real automaticamente
- Query "quem é Pro pagante?" continua sendo `WHERE planCode = pro AND planStatus = active`
- Métricas de MRR e conversão ficam corretas sem ajuste manual
- Reversível: tirar cortesia é só ajustar `planExpiresAt`

Função única de checagem de acesso:

```
canPractice(user) {
  if (user.planStatus !== 'active') return false
  if (user.planExpiresAt < now) return false
  return true
}
```

Independe se é trial ou pro. Ativo + dentro do prazo = pratica.

Quando bater complexidade (mais de 20-30 cortesias, múltiplos canais de pagamento, integração com Stripe), evolui pra schema mais explícito com status `lifetime` separado e campo `notes`. Por enquanto, simples é mais robusto.
...
- [ ] Limites do trial: 1 dia, conta trancada após expiração (planExpiresAt < now)
...
- [ ] Irmão e cunhado testando 5 dias com material real
...
- [ ] 30 dias de trial estendido pra eles (planExpiresAt = createdAt + 30d)
...
**Meta de independência:** R$25.000 líquido/mês para sair do emprego.
...
Conversão trial → pago no novo modelo deve ser maior que freemium tradicional, porque:

- Trial é 1 dia com experiência completa, não tier permanente — pressão natural de upgrade
- Posicionamento de "complementar", não substituir, reduz objeção
- Indicação de professor traz lead pré-qualificado (e pode vir com trial estendido de 3 dias)

Estimativa realista: 8-12% conversão. Se cair pra 5-7% (cenário pessimista), meta sobe pra ~3.000 pagantes.

Base de trials necessária: **15.000-22.000 usuários** que testam o produto. Bem mais factível que os 100k do brief anterior.
...
- **Trial:** 1 dia com experiência completa de Pro, conta trancada após expirar; trial estendido (3-7 dias) como ferramenta de campanha; sem free permanente
...
- **Sem freemium permanente:** trial finito de 1 dia com experiência plena, em vez de tier gratuito permanente que vira freemium trap
