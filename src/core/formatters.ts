export function formatTagList(tags: { name: string; noteCount: number }[]): string {
  if (tags.length === 0) {
    return 'Você ainda não tem tags. Adicione _#tags_ às suas notas!'
  }
  const lines = tags.map(t => `• #${t.name} (${t.noteCount} nota${t.noteCount !== 1 ? 's' : ''})`)
  return `*Suas tags:*\n${lines.join('\n')}`
}
...
export function formatSearchResults(notes: { content: string }[], query: string): string {
  if (notes.length === 0) {
...
export function formatNoteSaved(tags: string[], totalDailyCount: number): string {
...
export function formatUpgradePrompt(
  reason: 'audio' | 'image' | 'tag_limit' | 'search' | 'daily_limit' | 'history',
): string {
  const messages: Record<typeof reason, string> = {
    audio: 'Transcrição de áudio é um recurso Pro. Faça upgrade para desbloquear! 🎙️',
    image: 'Extração de texto de imagens é um recurso Pro. Faça upgrade para desbloquear! 🖼️',
    tag_limit:
...
export function formatReferralMessage(referralCode: string): string {
  return `Convide amigos para o KeeperZap e ganhe benefícios!\n\nSeu código de indicação: \`${referralCode}\``
}
